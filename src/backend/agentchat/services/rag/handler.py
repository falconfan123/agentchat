...
class RagHandler:
    ...
    async def rag_query_summary(cls, query, knowledges_id, min_score: Optional[float]=None,
        ...
        if needs_query_rewrite:
            ...
            if event_callback:
                event_callback({
                    "status": "END",
                    "title": "RAG: 查询改写",
                    "message": f"已将查询改写为 {len(rewritten_queries)} 个子查询: {', '.join(rewritten_queries)}"
                })
        else:
        ...
        if event_callback:
            event_callback({
                "status": "END",
                "title": "RAG: 文档检索",
                "message": f"从知识库检索到 {len(retrieved_documents)} 个相关文档"
            })
        ...
        if event_callback:
            event_callback({
                "status": "END",
                "title": "RAG: 重排序",
                "message": f"已完成文档重排序，返回 top-{len(reranked_docs)} 个结果"
            })
        ...
        if len(reranked_docs) >= actual_top_k:
            ...
            return await cls.retrieve_ranked_documents(query, knowledges_id, knowledges_id, event_callback=event_callback)
    ...
    async def retrieve_ranked_documents(cls, query, collection_names, index_names=None, min_score: Optional[float]=None,
        ...
        Args:
            ...
            event_callback (Callable): 事件回调函数，用于发送RAG流程中的事件
